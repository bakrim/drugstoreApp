...
/**
 * REST controller for managing {@link com.drugstore.app.domain.Document}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Document> partialUpdateDocument(
        ...
        Optional<Document> result = documentRepository
            ...
            .map(existingDocument -> {
                ...
                if (document.getThumbnailName() != null) {
                    existingDocument.setThumbnailName(document.getThumbnailName());
                }
                if (document.getMimeType() != null) {
                    existingDocument.setMimeType(document.getMimeType());
                }
                if (document.getThumbnailPath() != null) {
                    existingDocument.setThumbnailPath(document.getThumbnailPath());
                }
                if (document.getDownloadPath() != null) {
                    existingDocument.setDownloadPath(document.getDownloadPath());
                }
                if (document.getThumbnailDownloadPath() != null) {
                    existingDocument.setThumbnailDownloadPath(document.getThumbnailDownloadPath());
                }
